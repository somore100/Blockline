"""
Blockliner Build Script (cross-platform)
=========================================
Builds a distributable binary for whichever OS it's run on:

  - Windows -> Blockliner.exe                (PyInstaller --onefile --windowed)
  - Linux   -> dist/Blockliner (ELF binary)   (PyInstaller --onefile)
              + Blockliner-x86_64.AppImage    (wraps the binary)
  - macOS   -> dist/Blockliner (onefile binary; .app bundling is a separate step)
...
# V2: the old top-level shared-core .py "blocks" folder is gone -
# each language pack now has its own languages/<lang>/blocks/*.json
...
def existing_data_files():
    """Only bundle data files that actually exist, and warn about the rest."""
    found = []
    for name in EXTRA_DATA_FILES:
        ...
        if p.is_file():
            ...
            print(f"⚠ Data file '{name}' not found next to build.py, skipping")
    return found
...
def run_pyinstaller(windowed: bool, icon_path: Path | None):
    ...
    for f in existing_data_files():
        cmd.append(f"--add-data={ROOT / f}{sep}.")
...
def main():
    parser = argparse.ArgumentParser(description="Build Blockliner for this platform.")
    parser.add_argument(
        "--target",
